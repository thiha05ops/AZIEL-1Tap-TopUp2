...
const readyStoreActivationTargets=item=>(item?._activationTargets||[]).filter(target=>target.ready===true);
const storeActivationRequestTargets=item=>readyStoreActivationTargets(item).map(({mappingId,customerMarket,sourceLock})=>({mappingId,customerMarket,sourceLock}));
const storeSellingMarketOptions=Object.freeze([["TH","Thailand"],["MM","Myanmar"]]);
...
function renderStoreSellingMarkets(item){return`<section class="wizard-review-list"><h4>Selling Markets</h4><div>${storeSellingMarketOptions.map(([region,label])=>`<label><input type="checkbox" data-store-selling-region="${region}" data-selection-id="${storeEscape(storeId(item))}" data-version="${item.decisionVersion}" ${(item.sellingRegions||[]).includes(region)?"checked":""}> ${label}</label>`).join("")}</div><small>Controls AZIEL TH/MM commerce setup only. Product/account market and supplier route stay unchanged.</small></section>`}
function renderStoreCatalogPackages(){const element=document.getElementById("activationPackages"),item=storeCatalogAdmin.selections.find(row=>storeId(row)===storeCatalogAdmin.activeId);if(!element)return;if(!item){element.innerHTML='<div class="admin-empty-state">Select or add a Store Catalog product.</div>';return}element.innerHTML=`<header class="activation-summary"><div><span class="admin-status-pill">Store Catalog</span><h3>${storeEscape(item.productName)} (${item.supplierMarket==="GLOBAL"?"Global":storeEscape(item.supplierMarket)})</h3><p>Selling regions: ${(item.sellingRegions||[]).map(storeMarketName).join(" · ")} · Supplier: ${storeEscape(item.supplierCode)}</p></div><div class="business-inline-actions"><button class="admin-primary-btn" type="button" data-store-pricing>Set Prices</button><button class="admin-secondary-btn" type="button" data-store-storefront>Open Storefront</button></div></header>${renderStoreSellingMarkets(item)}<div class="store-catalog-package-list">${(item.packages||[]).map(pkg=>`<article class="activation-package"><div><strong>${storeEscape(pkg.packageName)}</strong><code>${storeEscape(pkg.packageCode)}</code></div>${(item.sellingRegions||[]).map(market=>`<div class="store-package-business-readiness" data-store-activation-card data-mapping-id="${storeEscape(storeId(pkg.supplierProductMappingId))}" data-package-code="${storeEscape(pkg.packageCode)}" data-market="${storeEscape(market)}"><span>Checking ${storeEscape(storeMarketName(market))} readiness…</span></div>`).join("")}<button class="admin-secondary-btn danger" type="button" data-store-remove-package="${storeEscape(pkg.packageCode)}">Remove from Product</button></article>`).join("")}</div><p class="wizard-business-note">Removing a package never deletes its Master Catalog mapping, supplier history, prices, or past orders.</p>`;loadStorePackageActivationCards(item).catch(()=>{})}
const storeMarketName=market=>market==="TH"?"Thailand":market==="MM"?"Myanmar":market;
async function loadStorePackageActivationCards(item){item._activationTargets=[];item._blockedActivationTargets=[];const businessRoot=blockers=>blockers[0]||"Technical preparation is required";for(const card of document.querySelectorAll("[data-store-activation-card]")){const market=card.dataset.market,marketName=storeMarketName(market);try{const result=await adminFetch(`/api/admin/store-catalog-selections/${encodeURIComponent(storeId(item))}/packages/${encodeURIComponent(card.dataset.mappingId)}/activation?customerMarket=${encodeURIComponent(market)}`);card._activationSourceLock=result.sourceLock;if(result.state==="ROUTE_ACTIVE"){card.innerHTML=`<strong>ROUTE ACTIVE FOR ${storeEscape(marketName.toUpperCase())}</strong><small>Set and publish the retail price next; Storefront visibility remains separate.</small>`;continue}const target={mappingId:card.dataset.mappingId,packageCode:card.dataset.packageCode,customerMarket:market,sourceLock:result.sourceLock,ready:result.ready===true,blockers:[...(result.blockers||[])]};if(target.ready)item._activationTargets.push(target);else item._blockedActivationTargets.push(target);const root=businessRoot(target.blockers);card.innerHTML=`<strong>${target.ready?"Ready to activate":"Preparation required"}</strong><small>${target.ready?`${storeEscape(marketName)} fulfillment route is prepared.`:storeEscape(root)}</small>`}catch(error){item._blockedActivationTargets.push({mappingId:card.dataset.mappingId,packageCode:card.dataset.packageCode,customerMarket:market,ready:false,blockers:[error.message]});card.innerHTML=`<strong>Preparation required</strong><small>${storeEscape(error.message)}</small>`}}const host=document.querySelector("#activationPackages .activation-summary .business-inline-actions"),readyCount=item._activationTargets.length,blockedCount=item._blockedActivationTargets.length;if(host&&(readyCount||blockedCount)){host.insertAdjacentHTML("beforeend",`<button class="admin-primary-btn" type="button" data-store-start-selling ${readyCount?"":"disabled"}>Start Selling</button>${blockedCount?`<small title="Blocked routes remain inactive">${blockedCount} route target${blockedCount===1?"":"s"} require preparation; ${readyCount} ready to activate.</small>`:""}`)}}
