...
(function () {
    ...
    function resolveProductRoute(route = "", productCode = "") { const value = String(route || "").trim(); if (value && !/^(?:[a-z]+:|\/\/)/i.test(value) && !value.startsWith("javascript:")) return value; const code = String(productCode || "").trim().toLowerCase(); return code ? `/products/${encodeURIComponent(code)}` : "#"; }
