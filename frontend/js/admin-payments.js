        ...
        return `
            <div class="payment-method-card ${legacyThaiBank ? "is-legacy-thai-bank" : ""}" data-id="${escapeAdminHTML(method._id)}" data-key="${escapeAdminHTML(method.key || "")}" data-region="${escapeAdminHTML(method.region || "")}" data-configuration-kind="${escapeAdminHTML(method.configurationKind || "MANUAL_QR")}" data-legacy-thai-bank="${legacyThaiBank ? "true" : "false"}">
                ...
                <div class="settings-row">
                    ...
                    <input class="pm-description" type="text" value="${escapeAdminHTML(method.shortDescription || "")}" placeholder="Brief payment description">
                </div>
                ...
                <div class="settings-row">
                    ...
                    <input class="pm-badge" type="text" value="${escapeAdminHTML(method.badgeText || "")}" placeholder="Optional badge">
                </div>
                ...
                <div class="payment-capability-details payment-verification-section" ${method.applicableSections?.some(section => ["manualVerification", "automaticProvider"].includes(section)) ? "" : "hidden"}>
                    <h5>Verification</h5>
...
function getAdminPaymentReadiness(method = {}) {
    ...
    if (type !== "wallet" && type !== "auto") {
        ...
        if (method.qrMode === "aziel_promptpay_dynamic") {
            ...
            if (provider === "thunder_promptpay" && !String(method.accountNumber || "").trim()) missing.push("Thunder receiving bank account");
        }
    }
...
function getQrCustomerExperience(method = {}) {
    ...
    if (normalizeAdminProvider(method.provider || method.key) === "thunder_promptpay") return "Customer experience: AZIEL generates an amount-specific PromptPay QR; the uploaded slip is verified automatically by Thunder.";
...
function bindChecklistPresetActions() {
    ...
    document.querySelectorAll('[data-action="promptpay-preset"]').forEach(btn => {
        btn.onclick = () => {
            const card = btn.closest(".payment-method-card");
            const thunder = card?.dataset.key === "thunder_promptpay" || normalizeAdminProvider(card?.querySelector(".pm-provider")?.value) === "thunder_promptpay";
            applyChecklistPreset(card, thunder ? [
                { action: "save_qr", label: "Save QR" },
                { action: "upload_receipt", label: "Upload Payment Slip" },
                { action: "wait_for_confirmation", label: "Automatic Verification" }
            ] : [
                { action: "save_qr", label: "Save QR" },
...
async function saveAdminPaymentMethod(id) {
    ...
    if (card.dataset.key === "thunder_promptpay" || payload.provider === "thunder_promptpay") {
        ...
        payload.paymentType = "manual";
        payload.qrMode = "aziel_promptpay_dynamic";
        payload.confirmationMode = "thunder_slip";
        payload.receiptUploadEnabled = true;
        payload.slipRequired = true;
        payload.dynamicQrSupported = true;
        payload.amountPrefillSupported = true;
...
function autoPaymentConfig(card) {
    ...
    if (provider === "thunder_promptpay" || card.dataset.key === "thunder_promptpay") {
        type.value = "manual";
        qrUrl.style.display = "none";
        qrUpload.style.display = "none";
        uploadBtn.style.display = "none";
    }
    else if (provider === "promptpay" && card.dataset.key === "promptpay") {
