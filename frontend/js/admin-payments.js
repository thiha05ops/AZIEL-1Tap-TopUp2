        ...
        return `
            <div class="payment-method-card ${legacyThaiBank ? "is-legacy-thai-bank" : ""}" data-id="${escapeAdminHTML(method._id)}" data-key="${escapeAdminHTML(method.key || "")}" data-region="${escapeAdminHTML(method.region || "")}" data-configuration-kind="${escapeAdminHTML(method.configurationKind || "MANUAL_QR")}" data-legacy-thai-bank="${legacyThaiBank ? "true" : "false"}">
                ...
                <div class="settings-row">
                    ...
                    <input class="pm-description" type="text" value="${escapeAdminHTML(method.shortDescription || "")}" placeholder="${method.key === "tmw_promptpay" ? "PromptPay with automatic confirmation" : "Brief payment description"}">
                </div>
                ...
                <div class="settings-row">
                    ...
                    <input class="pm-badge" type="text" value="${escapeAdminHTML(method.badgeText || "")}" placeholder="${method.key === "tmw_promptpay" ? "Automatic" : "Optional badge"}">
                </div>
                ...
                ${method.key === "tmw_promptpay" ? `
                    <div class="payment-method-hint">
                        <strong>TMW PromptPay Automatic</strong>
                        <span>The provider generates the QR and confirms payment by webhook. Receipt upload and payment slips are disabled.</span>
                    </div>
                ` : ""}
...
function getPaymentTypeDescription(key, type, provider) {
    if (key === "tmw_promptpay" || normalizeAdminProvider(provider) === "tmw") {
        return "Provider-generated PromptPay QR with automatic webhook confirmation.";
    }
