...
const { StoreCatalogSelectionError, listStoreCatalogSelections, saveStoreCatalogSelection, removeStoreCatalogPackage, setStoreCatalogRegionVisibility, setStoreCatalogSellingRegions } = require("../services/storeCatalogSelectionService");
...
router.post("/admin/store-catalog-selections/:selectionId/start-selling",adminMiddleware,requireAdminPermission(PERMISSIONS.OWNER_ROUTING_MANAGE),async(req,res)=>{try{return res.json({success:true,...await require("../services/storePackageActivationService").activateStoreCatalogSelection({...req.body,selectionId:req.params.selectionId},{actor:req.admin,req})})}catch(error){return sendStorePackageActivationError(res,error)}});
...
router.patch("/admin/store-catalog-selections/:selectionId/selling-regions",adminMiddleware,requireAdminPermission(PERMISSIONS.CATALOG_MANAGE),async(req,res)=>{try{const result=await setStoreCatalogSellingRegions({selectionId:req.params.selectionId,sellingRegions:req.body?.sellingRegions,expectedDecisionVersion:req.body?.expectedDecisionVersion},{actor:req.admin});await writeAdminAudit({actor:req.admin,req,action:ADMIN_AUDIT_ACTIONS.STORE_CATALOG_SELLING_MARKETS_CHANGED,resourceType:"StoreCatalogSelection",resourceId:req.params.selectionId,metadata:{before:result.before,after:result.after}});return res.json({success:true,...result})}catch(error){return sendStoreSelectionError(res,error)}});
