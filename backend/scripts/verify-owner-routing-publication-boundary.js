...
assert(routing.includes("PRIMARY_ROUTE_CONFLICT"));
assert(routing.includes("replaceExistingPrimaryId"));
assert(routing.includes("PRIMARY_REPLACEMENT_ROLE_REQUIRED"));
assert(!routing.includes("{ $set: { productionRole: ROLES.BACKUP } }"), "PRIMARY replacement must never silently create BACKUP");
assert(routing.includes('productionRole: ROLES.PRIMARY'));
assert(fulfillment.includes("SUPPLIER_MAPPING_NOT_PRIMARY"));
assert(fulfillment.includes('mapping.productionRole !== "DISABLED"'));
assert(fulfillment.includes("Only the Owner can modify a production-role supplier mapping."));
