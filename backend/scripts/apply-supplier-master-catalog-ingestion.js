...
const { customerFieldForProviderField } = require("../services/suppliers/fazercardsFulfillmentContractService");
...
async function applySupplier(supplierCode, supplier, source, namespace) {
    ...
    const productOps = source.products.map(row => {
        ...
        const machineFields = (row.requiredCustomerInputs || []).map(field => ({ customerField: customerFieldForProviderField(field.key), providerField: field.key, required: true, label: field.label, evidenceSource: field.source })), preserveReview = old?.normalizedInputContract?.review?.status === "OWNER_REVIEWED" && !changed, requiredFields = preserveReview ? old.requiredFields : machineFields, normalizedInputContract = preserveReview ? old.normalizedInputContract : { fields: machineFields, authority: row.marketEvidenceCode, sourceHash: row.rawSnapshotHash, sourceRevision: source.sourceSetHash };
        return { updateOne: { filter: { supplierId: supplier._id, catalogNamespace: namespace, supplierProductCode: row.supplierProductCode }, update: { $set: { supplierMarketCode: row.supplierMarketCode, displayName: row.displayName, rawName: row.displayName, categoryCode: row.supplierProductCode, supportState: row.supportState, requiredFields, normalizedInputContract, restrictions: row.restrictions || [], metadata: { marketCoverageState: row.marketCoverageState, targetMarketEligible: row.targetMarketEligible, marketEvidenceCode: row.marketEvidenceCode, deliveryClass: row.deliveryClass, masterSourceSetHash: source.sourceSetHash }, lastSeenAt: observedAt, lastObservedAt: observedAt, lastChangedAt: changed ? observedAt : old.lastChangedAt, sourceRevision: source.sourceSetHash, rawSnapshotHash: row.rawSnapshotHash, rawSnapshot: row.rawSnapshot }, $setOnInsert: { supplierId: supplier._id, catalogNamespace: namespace, supplierProductCode: row.supplierProductCode, firstSeenAt: observedAt } }, upsert: true } };
    });
