...
assert(css.includes("grid-template-columns: 66px minmax(0, 1fr) 18px"), "mobile compact package selector must preserve larger preview image, text, and chevron columns");
assert(css.includes("min-height: 86px"), "mobile compact package selector must keep a comfortable commerce-row height");
...
assert(css.includes("width: 64px !important") && css.includes("height: 64px !important"), "mobile compact package selector preview image must have a dedicated artwork slot");
