...
const service=read("backend/services/storePackageActivationService.js"),routes=read("backend/routes/supplier.js"),ui=read("frontend/js/admin-guided-selling.js");
    const mutationStart=service.indexOf("mapping.productionRole=\"PRIMARY\""),publicationWrite=service.indexOf("await setPackageMarketPublication",mutationStart),visibilityWrite=service.indexOf("selection.visibleRegions=",publicationWrite);assert(mutationStart<publicationWrite);assert(publicationWrite<visibilityWrite);assert(service.includes("for(const market of group.markets)allowed.add(market)"),"A shared GLOBAL mapping must retain both TH and MM eligibility in one publish transaction.");assert(service.includes("const states=[];for(const target of input.targets)")&&service.indexOf("const states=[];for(const target of input.targets)")<service.indexOf("for(const group of byMapping.values())"),"Every submitted target must preflight before the first batch mutation.");assert(routes.includes("requireAdminPermission(PERMISSIONS.OWNER_ROUTING_MANAGE)"));assert(ui.includes("Publish / Start Selling")&&ui.includes("SELLING IN ${storeEscape(marketName.toUpperCase())}"));assert(ui.includes("item._blockedActivationTargets=[]")&&ui.includes("if(target.ready)item._activationTargets.push(target);else item._blockedActivationTargets.push(target)"),"Ready and blocked package markets must remain independently classified.");assert(ui.includes('data-store-start-selling ${readyCount?"":"disabled"}'),"One ready target must keep Publish / Start Selling enabled even when another target is blocked.");assert(ui.includes("targets:storeActivationRequestTargets(item)"),"Activation must submit the reviewed ready-target subset only.");assert(ui.includes("Blocked package markets will remain private."),"Blocked targets must remain explicitly private.");
