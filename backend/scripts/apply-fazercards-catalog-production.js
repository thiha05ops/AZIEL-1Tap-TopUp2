...
const accepted = require("../../docs/fazercards-sellable-catalog-audit.json");

const APPLY = process.argv.includes("--apply");
const SUPPORTED = Object.keys(svc.CATEGORY_CONFIG);
const EXPECTED_COUNTS = Object.freeze({ mobile_legends_global: 43, pubg_mobile_auto: 29, free_fire_th: 12, honor_of_kings: 16, valorant_th: 6 });
const EXPECTED_MARKETS = Object.freeze({ mobile_legends_global: "GLOBAL", pubg_mobile_auto: "GLOBAL", free_fire_th: "TH", honor_of_kings: "UNSPECIFIED", valorant_th: "TH" });
...
async function protectedState() {
    const models = { CatalogProduct, CatalogPackage, SupplierProductMapping: Mapping, PackageMarketPublication: Publication, PricingQuote, CommerceOrder, FulfillmentAttempt, PackageInventoryState };
...
function acceptedOfferSet() {
    return new Set(accepted.offers.filter(x => SUPPORTED.includes(x.category_id)).map(x => `${x.category_id}/${x.offer_id}`));
}

function assertFresh(stage) {
    ...
    assert.strictEqual(stage.products.length, 5, "All five supported products must normalize.");
    assert.strictEqual(stage.offers.length, 106, "Fresh offer total drifted from reviewed baseline.");
    const counts = Object.fromEntries(stage.categoryResults.map(x => [x.category, x.offersObserved]));
    assert.deepStrictEqual(counts, EXPECTED_COUNTS, "Supported category offer counts drifted.");
    assert(stage.categoryResults.every(x => x.complete), "Every supported category must be complete.");
    assert.deepStrictEqual(Object.fromEntries(stage.products.map(x => [x.supplierProductCode, x.supplierMarketCode])), EXPECTED_MARKETS, "Supplier market normalization drifted.");
    const fresh = new Set(stage.offers.map(identity));
    const prior = acceptedOfferSet();
    assert.deepStrictEqual({ added: setDiff(fresh, prior), removed: setDiff(prior, fresh) }, { added: [], removed: [] }, "Supported provider offer identities drifted from reviewed evidence.");
}
...
async function main() {
    ...
    assert.deepStrictEqual({ missing: setDiff(providerProducts, persistedProducts), extra: setDiff(persistedProducts, providerProducts) }, { missing: [], extra: [] });
    assert.deepStrictEqual({ missing: setDiff(providerOffers, persistedOffers), extra: setDiff(persistedOffers, providerOffers) }, { missing: [], extra: [] });
    assert.strictEqual(postCatalog.availability.length, stage.offers.length);
    assert(postCatalog.availability.every(x => x.state === "AVAILABLE" && x.evidenceCode === "FAZERCARDS_CATALOG_LISTED" && x.coverageComplete === true && String(x.observationRunId) === String(run._id)));
    assert.deepStrictEqual(Object.fromEntries(postCatalog.products.map(x => [x.supplierProductCode, x.supplierMarketCode])), EXPECTED_MARKETS);
