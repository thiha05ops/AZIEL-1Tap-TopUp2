...
async function main() {
    ...
    assert(manual.includes('confirmationMode: "manual_admin"')); assert(wallet.length > 0); assert(!/WonDD|FazerCards|ensurePaidOrderFulfillmentWork|paidFulfillmentRoutingService/.test(thunder)); assert(controller.includes('duplicate_unbound')); assert(controller.includes('delete payment._receiptUploadDisposition')); assert(frontend.includes('result.verificationStatus !== "pending"'));
