...
const normalizeSellingRegions = value => [...new Set((value || []).map(upper))].filter(region => ["TH", "MM"].includes(region)).sort();
...
function createStoreCatalogSelectionService(models = {}, dependencies = {}) {
    ...
    async function save(input = {}, context = {}) {
        ...
        const sellingRegions = normalizeSellingRegions(input.sellingRegions);
    ...
    async function setRegionVisibility(input={},context={}){const region=upper(input.region);if(!["TH","MM"].includes(region)||typeof input.visible!=="boolean")throw new StoreCatalogSelectionError("STORE_SELECTION_VISIBILITY_INVALID","Choose a valid selling region and visibility state.");const selection=await lean(M.Selection.findOne({_id:input.selectionId,status:"ACTIVE"}));if(!selection)throw new StoreCatalogSelectionError("STORE_SELECTION_NOT_FOUND","Store Catalog product not found.",404);if(!selection.sellingRegions.includes(region))throw new StoreCatalogSelectionError("STORE_SELECTION_REGION_NOT_SELECTED","This region is not part of the Store Catalog product.",409);if(Number(input.expectedDecisionVersion||0)!==Number(selection.decisionVersion||0))throw new StoreCatalogSelectionError("STORE_SELECTION_STALE","This Store Catalog product changed. Refresh it and try again.",409);const visibleRegions=[...new Set(input.visible?[...(selection.visibleRegions||[]),region]:(selection.visibleRegions||[]).filter(item=>upper(item)!==region))].map(upper).filter(item=>selection.sellingRegions.includes(item)).sort(),update={visibleRegions,decisionVersion:Number(selection.decisionVersion||0)+1,selectedBy:clean(context.actor?.username||context.actor||"admin"),selectedAt:new Date()};const updated=await M.Selection.findOneAndUpdate({_id:input.selectionId,status:"ACTIVE",decisionVersion:selection.decisionVersion},{$set:update},{new:true,runValidators:true});const finalSelection=updated?.lean?await updated.lean():updated;if(!finalSelection)throw new StoreCatalogSelectionError("STORE_SELECTION_STALE","This Store Catalog product changed. Refresh it and try again.",409);return{selection:finalSelection,pricesChanged:0,publicationDecisionsChanged:0,mappingsChanged:0}}
    async function setSellingRegions(input={},context={}){const sellingRegions=normalizeSellingRegions(input.sellingRegions);if(!sellingRegions.length)throw new StoreCatalogSelectionError("STORE_SELECTION_SELLING_MARKETS_INVALID","Choose at least one AZIEL selling market.");const selection=await lean(M.Selection.findOne({_id:input.selectionId,status:"ACTIVE"}));if(!selection)throw new StoreCatalogSelectionError("STORE_SELECTION_NOT_FOUND","Store Catalog product not found.",404);if(Number(input.expectedDecisionVersion||0)!==Number(selection.decisionVersion||0))throw new StoreCatalogSelectionError("STORE_SELECTION_STALE","This Store Catalog product changed. Refresh it and try again.",409);const before={sellingRegions:[...(selection.sellingRegions||[])].sort(),visibleRegions:[...(selection.visibleRegions||[])].sort(),packages:(selection.packages||[]).map(item=>({packageCode:item.packageCode,supplierProductMappingId:item.supplierProductMappingId}))},visibleRegions=[...new Set((selection.visibleRegions||[]).map(upper).filter(region=>sellingRegions.includes(region)))].sort(),update={sellingRegions,visibleRegions,decisionVersion:Number(selection.decisionVersion||0)+1,selectedBy:clean(context.actor?.username||context.actor||"admin"),selectedAt:new Date()};const updated=await M.Selection.findOneAndUpdate({_id:input.selectionId,status:"ACTIVE",decisionVersion:selection.decisionVersion},{$set:update},{new:true,runValidators:true});const finalSelection=updated?.lean?await updated.lean():updated;if(!finalSelection)throw new StoreCatalogSelectionError("STORE_SELECTION_STALE","This Store Catalog product changed. Refresh it and try again.",409);return{selection:finalSelection,before,after:{sellingRegions:[...(finalSelection.sellingRegions||[])].sort(),visibleRegions:[...(finalSelection.visibleRegions||[])].sort(),packages:(finalSelection.packages||[]).map(item=>({packageCode:item.packageCode,supplierProductMappingId:item.supplierProductMappingId}))},pricesChanged:0,publicationDecisionsChanged:0,mappingsChanged:0,supplierCalls:0}}
    return { list, save, removePackage, setRegionVisibility, setSellingRegions };
}

const service = createStoreCatalogSelectionService();
module.exports = { StoreCatalogSelectionError, createStoreCatalogSelectionService, listStoreCatalogSelections: service.list, saveStoreCatalogSelection: service.save, removeStoreCatalogPackage: service.removePackage, setStoreCatalogRegionVisibility:service.setRegionVisibility, setStoreCatalogSellingRegions:service.setSellingRegions };
