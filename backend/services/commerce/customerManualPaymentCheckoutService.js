...
async function loadManualPaymentMethod({ key, region }, dependencies = {}) {
    ...
    if (method.enabled !== true || !["manual", "deeplink"].includes(type) || (!trueWallet && method.confirmationMode !== "manual_admin") || paymentMethodCapabilityState(method).customerVisible !== true) {
...
function sessionFrom({ checkout, payment, method }) {
    ...
    return {
        ...
        paymentName: trueWallet ? "TrueMoney Wallet" : (instructions.title || method.method), paymentMethod: method.key, paymentType: method.paymentType, provider: trueWallet ? "THUNDER_TRUEWALLET" : "MANUAL_ADMIN", paymentChannel: trueWallet ? "TRUE_MONEY_WALLET" : "MANUAL_ADMIN", confirmationMode: trueWallet ? "thunder_truewallet_slip" : "manual_admin",
...
async function startCustomerManualPaymentCheckout(input = {}, context = {}, dependencies = {}) {
    ...
    try {
        checkoutResult = await (dependencies.checkoutFromQuote || checkoutFromQuote)({
            ...
            paymentSelection: { paymentMethodId: method.key, paymentChannel: trueWallet ? "TRUE_MONEY_WALLET" : "MANUAL_ADMIN" },
            ...
            validatePaymentMethod: async () => ({ allowed: true, paymentSnapshot: { paymentMethodId: method.key, paymentChannel: trueWallet ? "TRUE_MONEY_WALLET" : "MANUAL_ADMIN", provider: trueWallet ? "THUNDER_TRUEWALLET" : "MANUAL_ADMIN", providerType: "manual", flowType: trueWallet ? "thunder_truewallet" : "manual_admin", confirmationMode: trueWallet ? "thunder_truewallet_slip" : "manual_admin", nextAction: "OPEN_MANUAL_PAYMENT", paymentMethodBound: true, metadata: { methodName: method.method, region: method.region, confirmationMode: trueWallet ? "thunder_truewallet_slip" : "manual_admin" } }, nextAction: "OPEN_MANUAL_PAYMENT" }),
