...
function normalizePaymentDisplayKey(value) {
    return String(value || "")
        .toLowerCase()
        .trim()
...
function formatPaymentDisplayName(value, fallback = "") {
    const key = normalizePaymentDisplayKey(value);
...
function formatPaymentMethod(method = {}, fallback = "Payment") {
    return formatPaymentDisplayName(
        method.key ||
        ...
        method.provider ||
        "",
        formatPaymentDisplayName(method.method || method.name || method.paymentMethod || "", fallback)
    );
}

function replacePaymentDisplayNames(value = "") {
...
module.exports = {
    PAYMENT_DISPLAY_LABELS: LABELS,
    formatPaymentDisplayName,
    formatPaymentMethod,
    normalizePaymentDisplayKey,
