...
const DEFINITIONS = Object.freeze({
    mlbb: [family("DIAMONDS", "Diamonds", 10), family("FIRST_TOP_UP", "First Top-Up", 20), family("WEEKLY_PASS", "Weekly Pass", 30), family("TWILIGHT_PASS", "Twilight Pass", 40), family("OTHER_SPECIAL", "Other / Special", 90)],
    "mlbb-twilight-weekly-pass": [family("WEEKLY_PASS", "Weekly Pass", 10), family("TWILIGHT_PASS", "Twilight Pass", 20), family("OTHER_SPECIAL", "Other / Special", 90)],
    ...
    freefire: [family("DIAMONDS", "Diamonds", 10), family("LEVEL_UP_PASS", "Level Up Pass", 20), family("BP_CARD", "BP Card", 30), family("MEMBERSHIP_WEEKLY_LITE", "Membership · Weekly Lite", 40, "MEMBERSHIP"), family("MEMBERSHIP_WEEKLY", "Membership · Weekly", 41, "MEMBERSHIP"), family("MEMBERSHIP_MONTHLY", "Membership · Monthly", 42, "MEMBERSHIP"), family("OTHER_SPECIAL", "Other / Special", 90)],
    hok: [family("TOKENS", "Tokens", 10), family("CARDS_PASSES", "Cards / Passes", 20), family("OTHER_SPECIAL", "Other / Special", 90)],
...
function canonicalFamily(productCode, packageName) {
    ...
    else if (product === "freefire") code = /level \d+ up pass/i.test(name) ? "LEVEL_UP_PASS" : /^bp card$/i.test(name) ? "BP_CARD" : /weekly membership lite/i.test(name) ? "MEMBERSHIP_WEEKLY_LITE" : /weekly membership/i.test(name) ? "MEMBERSHIP_WEEKLY" : /monthly membership/i.test(name) ? "MEMBERSHIP_MONTHLY" : /diamonds?/i.test(name) ? "DIAMONDS" : code;
    else if (product === "hok") code = /tokens?/i.test(name) ? "TOKENS" : /card|pass/i.test(name) ? "CARDS_PASSES" : code;
